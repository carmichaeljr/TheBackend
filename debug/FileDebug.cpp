...
bool FileDebug::debugIterator(void){
	...
	for (File::const_iterator iter=testFile.cbegin(); iter!=testFile.cend(); iter++, i++){
		temp << i+1;
		rv&=(*iter==temp.str());
	...
	rv&=(i==5);
	File::const_iterator iter1=testFile.cbegin();
	File::const_iterator iter2=testFile.cbegin();
	iter1+=2;
	iter2+=2;
	rv&=(iter1==iter2);
